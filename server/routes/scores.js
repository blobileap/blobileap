...
router.post('/', authRequired, async (req, res) => {
  try {
    ...
    var updatedUser = await pool.query("SELECT total_games, best_score FROM users WHERE id=$1", [req.user.id]); res.json({ success: true, rank: rankQ.rows[0].rank, weeklyRank, rewardAmount, totalGames: parseInt(updatedUser.rows[0].total_games), bestScore: parseInt(updatedUser.rows[0].best_score || 0) });
  } catch (e) {
